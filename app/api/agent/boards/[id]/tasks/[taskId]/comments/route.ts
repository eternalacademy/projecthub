...
// POST — Agent adds a comment to a task
export async function POST(request: Request, { params }: { params: Promise<{ id: string; taskId: string }> }) {
  ...
  const supabase = createServiceClient();
  const { data, error } = await supabase.from("ph_comments").insert({
    task_id: taskId,
    author_id: auth.agent.id,
    author_type: "agent",
    author_name: auth.agent.name,
    content,
  }).select().single();
  ...
  await supabase.from("ph_activity_log").insert({
    ...
    action: "agent_commented", details: { content: content.substring(0, 100) },
  });
